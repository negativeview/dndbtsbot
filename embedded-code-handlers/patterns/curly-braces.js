...
function work2(cb, stateHolder, state, error, value) {
	...
	if ('result' in value) {
		...
		async.eachSeries(
			value.toLoopOver,
			work4.bind(this, stateHolder, state, insideNode),
			function(error) {
				afterNode = m.nodes[2];
				if (afterNode.work) {
...
function work4(stateHolder, state, insideNode, variableOverrides, cb) {
	var keys = Object.keys(variableOverrides);
	for (var i = 0; i < keys.length; i++) {
		state.variables[keys[i]] = variableOverrides[keys[i]];
	}

	insideNode.work(stateHolder, state, function(error, res) {
		return cb(error);
	});
}
